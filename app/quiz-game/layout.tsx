...
export default function QuizGameLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className={`quiz-scope ${tajawal.variable}`}>
      <header className="sticky top-0 z-50 glass">
        <nav className="max-w-6xl mx-auto flex items-center justify-between px-5 py-3">
          <Link href="/quiz-game" className="flex items-center gap-2.5 group">
          ...
          <div className="flex items-center gap-1">
            ...
            <Link href="/quiz-game" className="px-3 py-2 rounded-lg text-sm font-medium text-[var(--muted)] hover:text-[var(--fg)] hover:bg-[var(--surface)] transition-all">
              Home
            </Link>
            <Link href="/quiz-game/play" className="px-4 py-2 rounded-lg text-sm font-semibold bg-[var(--accent)] text-white hover:bg-[var(--accent-hover)] transition-all">
              Play Now
            </Link>
          </div>
        </nav>
