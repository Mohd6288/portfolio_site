...
import { getRank, estimateMaxScore, getLevel, xpForLevel } from "../_lib/game-engine";
import { getLeaderboard, addToLeaderboard, isTopScore, type LeaderEntry } from "../_lib/leaderboard";
import { CATEGORIES } from "../_lib/questions";
import { AR } from "../_lib/i18n";
  ...
  if (!results) {
    return (
      <div className="max-w-lg mx-auto px-5 py-32 text-center fade-up">
        ...
        <Link href="/quiz-game" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold text-white" style={{ background: "var(--accent)" }}>
          Go Home
        </Link>
      </div>
    );
    ...
    <div className="max-w-lg mx-auto px-5 py-12 fade-up relative">
          ...
          {Array.from({ length: 15 }).map((_, i) => (
            <div
              ...
              style={{
                ...
                opacity: 0.3,
                animation: `quiz-particleFloat ${6 + Math.random() * 8}s linear ${Math.random() * 5}s infinite`,
              }}
            />
          ))}
        </div>
      ...
      <div className="relative z-10">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Link
            href={`/quiz-game/play?category=${results.categoryId}`}
